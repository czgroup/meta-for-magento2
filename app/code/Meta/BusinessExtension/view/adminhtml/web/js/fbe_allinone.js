(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
    ...
    jQuery('#store').on('change', function() {
      ...
      window.facebookBusinessExtensionConfig.catalogId = jQuery(this).find(':selected').data('catalog-id');
      window.facebookBusinessExtensionConfig.accessToken = jQuery(this).find(':selected').data('access-token');
      ...
      var FBEFlowContainer = React.createClass({
        ...
        render: function render() {
          ...
          try {
            ...
            // Get the value of the 'od_url' parameter
            const subpage = urlParams.get('subpage') ?? 'setup';
            const catalogId = window.facebookBusinessExtensionConfig.catalogId;
            _this.consoleLog("Catalog ID --" + catalogId);
            const accessToken = window.facebookBusinessExtensionConfig.accessToken;
            _this.consoleLog("Access Token --" + accessToken);
            const url = this.state.installed ? `https://www.my-od.commercepartnerhub.com/commerce_extension?subpage=${subpage}&catalog_id=${catalogId}&access_token=${accessToken}` : window.facebookBusinessExtensionConfig.fbeLoginUrl + _this.queryParams();
            _this.consoleLog("URL --" + url);
            return React.createElement(
              'iframe',
              {
                src: url,
                style: {border:'none',width:'1100px',height:'700px'},
                id:'meta-commerce-iframe'
              }
            );
          } catch (err) {
            console.error(err);
    ...
    var displayFBModal = function displayFBModal() {
      ...
      window.addEventListener("message", (msg) => console.log(msg, 'parent'));
      window.addEventListener("click", (msg) => {
        let cwindow = document.getElementById('meta-commerce-iframe').contentWindow;
        cwindow.postMessage({
            diagnostics: [
            ...
            bannerMessage: "Hello from " + window.location
          }, "*");
        console.log(cwindow)
      });
